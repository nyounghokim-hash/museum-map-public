        ...
        <div className="bg-white dark:bg-neutral-900 border border-gray-100 dark:border-neutral-800 rounded-3xl p-6 shadow-sm">
            <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex gap-2">
                    <button type="button" onClick={addPresets} className="px-3 py-1.5 rounded-xl text-[10px] font-bold bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 hover:bg-blue-100 transition-colors">
                        프리셋 추가
                    </button>
            ...
            {value.length === 0 ? (
                <p className="text-xs text-gray-400 text-center py-6">아직 정보가 없어요. 프리셋이나 행을 추가해 주세요.</p>
            ) : (
...
export function ArtworksEditor({ value, onChange }: { value: any[]; onChange: (v: any[]) => void }) {
    ...
    return (
        <div className="bg-white dark:bg-neutral-900 border border-gray-100 dark:border-neutral-800 rounded-3xl p-6 shadow-sm">
            ...
            {value.length === 0 ? (
                <p className="text-xs text-gray-400 text-center py-6">아직 작품이 없어요. 추가 버튼으로 작품을 연결해 주세요.</p>
            ) : (
                    ...
                    {results.map((m) => (
                        <button key={m.id} type="button" onClick={() => addMuseum(m)}
                            ...
                            {m.imageUrl ? (
                                ...
                                <div className="w-8 h-8 rounded-lg bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center flex-shrink-0"><MuseumIcon className="w-4 h-4 text-blue-600" /></div>
                            )}
                            ...
                            <span className="text-[10px] text-blue-500 font-bold">+ 추가</span>
                        </button>
                    ))}
                </div>
            )}
            ...
            {selectedMuseums.length > 0 ? (
                <div className="space-y-2">
                    {selectedMuseums.map((m) => (
                        <div key={m.id} className="flex items-center gap-3 p-3 rounded-xl bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/20">
                            {m.imageUrl ? (
                                <img src={m.imageUrl} alt="" className="w-8 h-8 rounded-lg object-cover flex-shrink-0" onError={(e) => { e.currentTarget.src = '/logo.svg'; e.currentTarget.className = 'w-8 h-8 rounded-lg object-contain p-1 opacity-20 dark:invert dark:opacity-60'; }} />
                            ) : (
                                <div className="w-8 h-8 rounded-lg bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center flex-shrink-0 text-sm">🏛️</div>
                            )}
                            <div className="flex-1 min-w-0">
                                <p className="text-xs font-bold dark:text-white truncate">{museumDisplayName(m)}</p>
                                <p className="text-[10px] text-gray-400">{museumDisplayCity(m)}, {m.country}</p>
                ...
                <p className="text-xs text-gray-400 text-center py-4">아직 연결된 박물관이 없어요.</p>
            )}
        </div>
    );
}
                    ...
                    {results.map((a) => (
                        <button key={a.id} type="button" onClick={() => addArtwork(a)}
                            ...
                            {a.image ? (
                                ...
                                <div className="w-8 h-8 rounded-lg bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center flex-shrink-0"><FrameIcon className="w-4 h-4 text-blue-600" /></div>
                            )}
                            <div className="flex-1 min-w-0">
            ...
            {selectedArtworks.length > 0 ? (
                <div className="space-y-2">
                    {selectedArtworks.map((a) => (
                        <div key={a.id} className="flex items-center gap-3 p-3 rounded-xl bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/20">
                            {a.image ? (
                                <img src={a.image} alt="" className="w-8 h-8 rounded-lg object-cover flex-shrink-0" />
                            ) : (
                                <div className="w-8 h-8 rounded-lg bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center flex-shrink-0 text-sm">🖼️</div>
                ...
                <p className="text-xs text-gray-400 text-center py-4">아직 연결된 작품이 없어요. 검색해서 추가해 주세요.</p>
            )}
        </div>
    );
}
