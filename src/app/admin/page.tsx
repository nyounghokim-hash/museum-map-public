    ...
    const fetchAdminData = (silent = false) => {
        ...
        Promise.all(endpoints.map(e => fetch(e).then(r => r.json())))
            ...
            .catch((err) => {
                ...
                if (!silent) showAlert(err?.message || '데이터 조회에 실패했습니다. 잠시 후 다시 시도해주세요.');
                setLoading(false);
            });
    };
            ...
            <div className="fixed inset-0 bg-black/40 dark:bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                <div className="bg-white dark:bg-neutral-900 rounded-2xl p-8 w-full max-w-sm shadow-2xl relative">
                    ...
                    <input
                        ...
                        placeholder="비밀번호"
                        className="w-full border border-gray-200 dark:border-neutral-700 bg-white dark:bg-neutral-800 rounded-xl px-4 py-3 mb-3 focus:outline-none focus:ring-2 focus:ring-purple-500 text-sm dark:text-white"
                        autoFocus
                    />
                    {error && <p className="text-red-500 text-xs mb-3">{error}</p>}
                    <button
                        onClick={handleLogin}
                        className="w-full bg-purple-600 hover:bg-purple-700 text-white py-3 rounded-xl font-bold transition-colors active:scale-95 shadow-lg shadow-purple-600/20"
                    >
                        입장하기
                    </button>
                </div>
            ...
            {loading ? (
                ...
                <div className="animate-fadeIn">
                    ...
                    <div className="mt-6 mb-10">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-4">
                            <div onClick={() => setTab('ai')} className="bg-white dark:bg-neutral-900 p-4 rounded-2xl border border-gray-100 dark:border-neutral-800 shadow-sm cursor-pointer hover:shadow-md hover:scale-[1.02] transition-all active:scale-[0.98]">
                                ...
                                <div className="text-lg font-black text-purple-600 dark:text-purple-400">{aiUsage?.month?.requests?.toLocaleString() || 0}</div>
                        ...
                        <div className="bg-gradient-to-r from-gray-900 to-gray-800 dark:from-neutral-800 dark:to-neutral-900 rounded-2xl px-5 py-3.5 flex items-center justify-between">
                            ...
                            <button onClick={() => setTab('ai')} className="text-[9px] font-black text-purple-400 hover:text-purple-300 uppercase tracking-widest transition-colors">
                                상세 보기 →
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-12">
                        ...
                        <div onClick={() => setTab('analytics')} className="admin-metric-card admin-metric-card-purple p-5 cursor-pointer hover:scale-[1.02] transition-all active:scale-[0.98]">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-10">
                        ...
                        <div>
                            ...
                            {(!dashboardData?.recentFeedback || dashboardData.recentFeedback.length === 0) && (
                                <p className="text-sm text-gray-400 dark:text-neutral-600">접수된 피드백이 없습니다.</p>
                            )}
                            <button
                ...
                <div className="animate-fadeIn">
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-8">
                        ...
                        <div className="bg-white dark:bg-neutral-900 p-4 rounded-2xl border border-gray-100 dark:border-neutral-800 shadow-sm">
                            ...
                            <div className="text-2xl font-black text-purple-600">{dashboardData?.stats?.users || '-'}</div>
                        </div>
                    </div>
                        ...
                        {feedbacks.map((f: any) => (
                            <div key={f.id} className="border border-indigo-50 dark:border-indigo-900/20 rounded-2xl p-6 bg-white dark:bg-neutral-900 flex flex-col gap-4 shadow-sm hover:shadow-md transition-shadow">
                                ...
                                {f.reply && !replyingId?.startsWith(f.id) ? (
                                    <div className="bg-purple-50 dark:bg-purple-900/10 rounded-2xl p-4 border border-purple-100 dark:border-purple-800/20">
                                        <p className="text-[10px] font-black text-purple-600 dark:text-purple-400 uppercase tracking-widest mb-1">관리자 답변</p>
                                        ...
                                        <button
                                            ...
                                            className="mt-2 text-[10px] font-bold text-purple-500 hover:text-purple-700 transition-colors"
                                        >수정하기</button>
                                    </div>
                                ) : replyingId === f.id ? (
                                    <div className="space-y-2">
                                        <textarea
                                            ...
                                            className="w-full bg-gray-50 dark:bg-neutral-800 border-none rounded-2xl px-4 py-3 text-xs focus:ring-2 focus:ring-purple-500 outline-none dark:text-white min-h-[80px] leading-relaxed"
                                            autoFocus
                                        />
                                        <div className="flex gap-2">
                                            <button
                                                ...
                                                className="px-4 py-2 bg-purple-600 text-white rounded-xl text-[11px] font-black hover:bg-purple-700 active:scale-95 transition-all"
                                            >저장</button>
                                            <button
                    ...
                    <div className="flex justify-between items-center mb-8">
                        ...
                        <div className="flex items-center gap-4">
                            <select
                                ...
                                className="bg-gray-50 dark:bg-neutral-800 border-none rounded-xl px-4 py-2.5 text-[11px] font-black uppercase tracking-widest outline-none focus:ring-2 focus:ring-purple-500 transition-all"
                            >
                                <option value="views">조회수순</option>
                                <option value="date">최신순</option>
                            </select>
                            <button onClick={() => router.push('/blog/new')} className="bg-purple-600 hover:bg-purple-700 text-white px-6 py-3 rounded-2xl text-xs font-black shadow-xl shadow-purple-600/20 hover:scale-105 active:scale-95 transition-all uppercase tracking-widest">스토리 작성</button>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 gap-4">
                        {[...posts].sort((a, b) => {
                            ...
                            const catConfig: Record<string, { emoji: string; label: string; bg: string; text: string }> = {
                                ...
                                ART: { emoji: '🎨', label: '아트', bg: 'bg-purple-50 dark:bg-purple-900/20', text: 'text-purple-700 dark:text-purple-400' },
                    ...
                    <div className="mb-8 flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
                        <form onSubmit={handleMuseumSearch} className="relative w-full sm:max-w-md">
                            <input
                                ...
                                className="w-full bg-white dark:bg-neutral-900 border border-gray-100 dark:border-neutral-800 rounded-2xl px-6 py-4 text-sm focus:ring-2 focus:ring-purple-500 outline-none dark:text-white shadow-sm"
                            />
                            <button type="submit" aria-label="미술관 검색" className="absolute right-4 top-4 text-gray-400 focus:outline-none focus:ring-2 focus:ring-purple-500 rounded">
                        ...
                        <button
                            ...
                            className="bg-purple-600 hover:bg-purple-700 text-white px-8 py-4 rounded-2xl text-sm font-black shadow-xl shadow-purple-600/20 hover:scale-105 active:scale-95 transition-all uppercase tracking-widest"
                        >
                            미술관/박물관 추가
                        </button>
                    </div>
                            ...
                            <tbody className="divide-y divide-gray-50 dark:divide-neutral-800">
                                {sortedMuseums.map((m) => (
                                    <tr key={m.id} className="hover:bg-gray-50/50 dark:hover:bg-neutral-800/50 transition-colors">
                                        ...
                                        <td className="px-8 py-5 text-[10px] tracking-widest uppercase font-black text-purple-600 dark:text-purple-400">{({ MUSEUM: '박물관', GALLERY: '미술관', SCIENCE_CENTER: '과학관', HISTORIC_SITE: '유적지', OTHER: '기타' } as any)[m.type] || m.type}</td>
                                        <td className="px-8 py-5">
                                            <div className="flex items-center gap-2">
                    ...
                    <div className="bg-white dark:bg-neutral-900 p-6 rounded-2xl border border-gray-100 dark:border-neutral-800 shadow-sm mb-8">
                        ...
                        <div className="flex items-center gap-4 mb-4">
                            <div className="flex items-center gap-1.5">
                                <div className="w-3 h-3 rounded-sm bg-gradient-to-t from-purple-500 to-purple-400" />
                                            ...
                                            return (
                                                <div key={d.date} className="flex-1 flex flex-col items-center gap-1 group cursor-default">
                                                    <div className="text-center mb-0.5">
                                                        <span className="text-[9px] font-black text-purple-600 dark:text-purple-400 block">{d.requests}</span>
                                                    ...
                                                    <div className="w-full rounded-t-lg relative overflow-hidden transition-all group-hover:opacity-90" style={{ height: `${barH}%` }}>
                                                        <div className="absolute inset-0 bg-gradient-to-t from-purple-600 to-purple-400 rounded-t-lg opacity-80" />
                                                    </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                            <div className="bg-white dark:bg-neutral-900 rounded-2xl border border-gray-100 dark:border-neutral-800 shadow-sm p-6">
                                <div className="flex items-center gap-3 mb-4">
                                    <div className="w-10 h-10 rounded-2xl bg-purple-50 dark:bg-purple-900/20 flex items-center justify-center">
                                ...
                                <div className="space-y-2 text-xs">
                                    <div className="flex justify-between"><span className="text-gray-400">30일 호출</span><span className="font-bold text-purple-600">{(aiUsage?.month?.requests || 0).toLocaleString()}회</span></div>
                        ...
                        <form onSubmit={handleSendNotification} className="bg-white dark:bg-neutral-900 border border-gray-100 dark:border-neutral-800 rounded-2xl p-8 shadow-sm space-y-6">
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full bg-gray-50 dark:bg-neutral-800 border-none rounded-2xl px-5 py-4 text-sm focus:ring-2 focus:ring-purple-500 outline-none dark:text-white font-bold"
                            ...
                            <div>
                                ...
                                <textarea
                                    ...
                                    className="w-full bg-gray-50 dark:bg-neutral-800 border-none rounded-2xl px-5 py-4 text-sm focus:ring-2 focus:ring-purple-500 outline-none dark:text-white min-h-[120px] leading-relaxed"
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full bg-gray-50 dark:bg-neutral-800 border-none rounded-2xl px-5 py-4 text-sm focus:ring-2 focus:ring-purple-500 outline-none dark:text-white font-mono"
                            ...
                            <div className="flex items-center justify-between bg-gray-50 dark:bg-neutral-800 rounded-2xl px-5 py-4">
                                ...
                                <button
                                    ...
                                    className={`relative w-11 h-6 rounded-full transition-colors duration-200 ${notifForm.marketingOnly ? 'bg-purple-600' : 'bg-gray-300 dark:bg-neutral-600'}`}
                                >
                            ...
                            <button
                                type="submit"
                                className="w-full bg-purple-600 text-white py-4 rounded-2xl font-black shadow-xl shadow-purple-600/20 hover:bg-purple-700 hover:scale-[1.02] active:scale-95 transition-all uppercase tracking-widest"
                            >
                                알림 보내기
                            </button>
                        </form>
                                    ...
                                    <div className="bg-white dark:bg-neutral-900 rounded-2xl border border-gray-100 dark:border-neutral-800 p-6 shadow-sm">
                                        ...
                                        <div className="space-y-3">
                                            {gaData.devices.map((d: any) => {
                                                ...
                                                const DC: Record<string, string> = { 'desktop': 'bg-blue-500', 'mobile': 'bg-purple-500', 'tablet': 'bg-emerald-500' };
                                                return (
                                    ...
                                    <div className="bg-white dark:bg-neutral-900 rounded-2xl border border-gray-100 dark:border-neutral-800 p-6 shadow-sm">
                                        ...
                                        <div className="space-y-3">
                                            {gaData.ageBrackets.map((a: any) => {
                                                ...
                                                const AC: Record<string, string> = { '18-24': 'from-green-500 to-emerald-400', '25-34': 'from-blue-500 to-cyan-400', '35-44': 'from-purple-500 to-violet-400', '45-54': 'from-amber-500 to-yellow-400', '55-64': 'from-orange-500 to-red-400', '65+': 'from-rose-500 to-pink-400', 'unknown': 'from-gray-400 to-gray-300' };
                                                return (
                                                    <div key={a.age}>
                                                        <div className="flex items-center justify-between text-xs mb-1">
                    ...
                    <div className="mb-8 flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
                        <form onSubmit={(e) => { e.preventDefault(); setArtworkPage(1); fetchAdminData(); }} className="relative w-full sm:max-w-md">
                            <input
                                ...
                                className="w-full bg-white dark:bg-neutral-900 border border-gray-100 dark:border-neutral-800 rounded-2xl px-6 py-4 text-sm focus:ring-2 focus:ring-purple-500 outline-none dark:text-white shadow-sm"
                            />
                            <button type="submit" aria-label="작품 검색" className="absolute right-4 top-4 text-gray-400 focus:outline-none focus:ring-2 focus:ring-purple-500 rounded">
                                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true"><path d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
                            </button>
                        </form>
                        <button
                            ...
                            <div key={aw.id} className="flex items-center gap-3 bg-white dark:bg-neutral-900 border border-gray-100 dark:border-neutral-800 rounded-xl px-4 py-3 hover:shadow-sm transition-all group">
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-2">
                                        ...
                                        {aw.artistKo || aw.artist ? <span className="text-[11px] text-purple-600 dark:text-purple-400 font-bold flex-shrink-0">{aw.artistKo || aw.artist}</span> : null}
                                    </div>
                                    <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                                        {aw.year && <span className="text-[10px] text-gray-400">{aw.year}</span>}
                                        {aw.museum && (
                                            <span className="inline-flex items-center gap-0.5 text-[9px] text-purple-600 dark:text-purple-400 bg-purple-50 dark:bg-purple-900/20 px-2 py-0.5 rounded-full font-bold">
                        ...
                        onSave={async (data: any) => {
                            try {
                                ...
                                if (res.ok) {
                                    ...
                                    const err = await res.json();
                                    showAlert(err.message || '작품 저장에 실패했습니다.');
                                }
                            } catch (err) {
